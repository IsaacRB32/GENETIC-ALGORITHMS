...
def funcion_penalizacion(x):
    ...
    pesos = np.array(x)
    retorno_actual = np.dot(pesos, rendimientos)
    
    ## Restricción: El retorno debe ser al menos 35% (0.35) -> 0.35 - retorno <= 0 (despeje)
    g1 = 0.35 - retorno_actual
    Rd = np.array([g1]) 
    
    ## La suma de todas las fracciones de inversión debe ser exactamente 1
...
print(f"Iniciando {ejecuciones} ejecuciones para el Problema 2...")
...
## CÁLCULO DE MÉTRICAS DE LA EJECUCIÓN ACTUAL
    ...
    # Queremos el que tenga el menor riesgo posible
    if mejor_apt < mejor_apt_global:
        mejor_apt_global = mejor_apt
        mejor_solucion_global = mejor_ind_e.copy()
        
    print(f" > Ejecución {e} terminada. Menor Riesgo encontrado: {mejor_apt:.6f}")

## Dibujamos una tabla con el reporte estadístico
print("\n" + "="*80)
print(f"{'Ejecución':<12} | {'Menor Riesgo':<15} | {'Promedio':<12} | {'Peor':<12} | {'Desv. Est.':<12}")
print("-" * 80)
...
print("\n*** SOLUCIÓN ÓPTIMA OBTENIDA (PROB 2: RIESGO MÍNIMO) ***")
...
print(f"RIESGO MÍNIMO ASOCIADO (VARIANZA): {riesgo_total:.6f}")
print("*"*60)