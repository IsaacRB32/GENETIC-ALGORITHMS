...
def funcion_penalizacion(x):
    ...
    ## Calculamos el riesgo (Varianza) actual de la cartera
    riesgo_actual = np.dot(pesos.T, np.dot(S, pesos))
    
    ## Forma g(x) <= 0  ->  Riesgo - 0.02 <= 0
    g1 = riesgo_actual - 0.02
    Rd = np.array([g1]) 
...
print(f"Iniciando {ejecuciones} ejecuciones para el Problema 3...")
...
## CÁLCULO DE MÉTRICAS DE LA EJECUCIÓN ACTUAL
    # Volvemos a la lógica de retorno (valores negativos)
    mejor_apt_real = np.min(aptitud) 
    ...
    tabla_metricas.append([e, mejor_apt_real, promedio_apt, peor_apt, desviacion_apt])
    
    ## Guardamos al mejor de los mejores (Global)
    # Buscamos el "más negativo" (el que da más ganancia)
    if mejor_apt_real < mejor_apt_global:
        mejor_apt_global = mejor_apt_real
        mejor_solucion_global = mejor_ind_e.copy()
        
    print(f" > Ejecución {e} terminada. Mejor retorno encontrado: {-mejor_apt_real*100:.2f}%")
...
print("\n*** SOLUCIÓN ÓPTIMA OBTENIDA (PROB 3: MAX RETORNO CON RIESGO <= 0.02) ***")
acciones = ["BB", "LOP", "ILI", "HEAL", "QUI", "AUA"]
for i in range(len(acciones)):
    print(f"Peso {acciones[i]}: {pesos[i]*100:.2f}%")

print(f"\nRETORNO MÁXIMO DE LA CARTERA: {retorno_total*100:.2f}%")
