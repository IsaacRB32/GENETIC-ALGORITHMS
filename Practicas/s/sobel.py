...
def energia_sobel(imagen_tran):
    
    ## Si la imagen es a color, convertir a grises
    if len(imagen_tran.shape) == 3:
        ## Convertir a escala de grises (promedio ponderado)
        # Fórmula: 0.2989*R + 0.5870*G + 0.1140*B
        img_float = 0.2989 * imagen_tran[:,:,0] + 0.5870 * imagen_tran[:,:,1] + 0.1140 * imagen_tran[:,:,2]
    else:
        img_float = imagen_tran.astype(np.float32)
    
    # Asegurar que los valores estén en [0,1]
    if img_float.max() > 1:
        img_float = img_float / 255.0
    
    ## Sobel horizontal (operador Sobel)
    sobel_x = np.array([[-1, 0, 1],
                        [-2, 0, 2],
                        [-1, 0, 1]])
    ## Sobel vertical (operador Sobel)
    sobel_y = np.array([[-1, -2, -1],
                        [0, 0, 0],
                        [1, 2, 1]])
    
    ## Aplicar convolución
    grad_x = convolve(img_float, sobel_x)
    grad_y = convolve(img_float, sobel_y)
    
    # Magnitud del gradiente
    magnitud_grad = np.sqrt(grad_x**2 + grad_y**2)
    
    # 2. SE NORMALIZAN (0-1) ENTRE EL MAYOR (Paso clave del profesor)
    # Buscamos el valor máximo de los gradientes en ESTA imagen
    max_grad = np.max(magnitud_grad)
    
    if max_grad > 0:
        # Dividimos toda la imagen entre su propio máximo
        magnitud_grad = magnitud_grad / max_grad
    
    # 3. SUMO TODO (Sin elevar al cuadrado y sin dividir por el size)
    energia = np.sum(magnitud_grad)
...
def evaluarPoblacion(imagen_original, poblacion):
    ...
    for i in range(len(poblacion)):
        ...
        aptitudes[i] = energia_sobel(img_transformada)
...
def algoritmoGenetico(limites, tam_poblacion, generaciones, p_cruzamiento, p_mutacion, nc, nm, ruta):
    ...
    energia_poblacion = evaluarPoblacion(imagen, poblacion)

    for i in range(generaciones):

        ## Seleccion de padres por torneo determinista
        padres = selecPadres(energia_poblacion, poblacion)
        ...
        ## Evaluar energía de los hijos mutados
        energia_hijos = evaluarPoblacion(imagen, hijos_mutados)

        ## Sustitución elitista
        poblacion = sustitucionExEl(poblacion, hijos_mutados, energia_hijos, energia_poblacion)

        ## Evaluar energía de la nueva población
        energia_poblacion = evaluarPoblacion(imagen, poblacion)

        mejor_individuo = poblacion[np.argmax(energia_poblacion)]
        mejor_energia = np.max(energia_poblacion)

        print(f"Generación {i+1}: Mejor individuo = {mejor_individuo}, Energía Sobel = {mejor_energia:.6f}")
...
mejor_individuo, mejor_energia = algoritmoGenetico(limites, tam_poblacion, generaciones, p_cruzamiento, p_mutacion, nc, nm, ruta)
...
if len(imagen.shape) == 3:
    print(f"   Tipo: Imagen a color (RGB)")
else:
    print(f"   Tipo: Imagen en escala de grises")

print(f"\nMejor solución encontrada:")
print(f"   Alpha = {mejor_individuo[0]:.4f}")
print(f"   Delta = {mejor_individuo[1]:.4f}")
print(f"   Energía Sobel final = {mejor_energia:.6f}")
...
cv2.imwrite("imagen_optima_sobel_White blood cell (basophil).jpg", imagen_guardar)
