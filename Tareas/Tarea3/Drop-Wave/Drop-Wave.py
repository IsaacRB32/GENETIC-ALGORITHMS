...
def graficar_poblacion(poblacion, mejor, gen, li, ls):
    ...
    Z = np.array([
        [funcionDropWave([X[j,i], Y[j,i]]) for i in range(len(x))]
...
def evaluar(poblacion):
    ...
    for individuo in poblacion:
        ##Indivio actual
        valor = funcionDropWave(individuo)
...
def funcionDropWave(x):
    # Calcula la distancia al origen
    r = math.sqrt(x[0]**2 + x[1]**2)
    
    numerador = 1 + math.cos(12 * r)
    denominador = 0.5 * (x[0]**2 + x[1]**2) + 2
    
    # Se retorna negativo porque la función original busca el mínimo
