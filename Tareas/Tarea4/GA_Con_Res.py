...
def graficar_poblacion(poblacion, mejor, gen, li, ls):
    ...
    Z = np.array([
        [funcion_objetivo([X[j,i], Y[j,i]]) for i in range(len(x))]
    ...
    ## Se dibujar las rectas de restricción
    ## y <= 2 - 2x
    y1 = 2 - 2*x
    ## y <= (6 - 3x) / 4
    y2 = (6 - 3*x) / 4
    
    plt.plot(x, y1, 'r--', label='2x + y <= 2')
    plt.plot(x, y2, 'b--', label='3x + 4y <= 6')
...
def cruzamientoSBX(padres, Pc, li, ls, Nc=50):
